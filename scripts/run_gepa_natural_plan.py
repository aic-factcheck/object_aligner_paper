"""NATURAL PLAN optimization: GEPA with an OA-based reward on Trip / Meeting.

A thin sibling of ``scripts/run_gepa_scierc.py`` (same arms, adapter, holdout,
resume — all reused unchanged). The experiment contrasts OA's order-aware list
alignment (``--schema .../trip_fixed.jsonc``) against the order-blind Hungarian
ablation (``.../trip_hungarian.jsonc``), exactly as SciERC contrasts RA vs
strict; run both arms back-to-back and cross-score (see ``run_natural_plan.sh``).

Three arms selected by ``--arm``:

  * ``oa_score``    — Score=OA; feedback="score: X.XXX/1.0" only.
  * ``oa_feedback`` — Score=OA; feedback=OA's prescriptive critique.
  * ``oa_none``     — Score=OA; feedback is a fixed placeholder.

The task (trip / meeting) is inferred from the schema file's stem, which sets
the default split-dir and seed prompt. The official binary exact-match metric
(for SOTA comparison) is computed separately by ``scripts/eval_natural_plan.py``.
...
def _task_from_schema(schema_path: Path) -> str:
    """Infer 'trip' or 'meeting' from the schema file stem."""
...
def main() -> None:
    ...
    p.add_argument("--schema", type=Path, default=None, metavar="PATH",
                   help="OA schema (.jsonc). The stem (trip_fixed/trip_hungarian/"
                        "meeting_fixed/meeting_hungarian) selects the task, is the "
    ...
    p.add_argument("--seed-prompt", type=Path, default=None, metavar="PATH",
                   help="Seed system prompt (default: the task's seed_prompt.txt).")
    p.add_argument("--config", type=Path, default=None,
                   help="LM config YAML (default: config/experiment.yaml or "
                        "$OAEXP_CONFIG).")
    p.add_argument("--split-dir", type=Path, default=None,
                   help="Dir with train/val/test.jsonl (default: the task's "
                        "gepa_*_planning/main).")
    p.add_argument("--run-name", type=str, default=None,
                   help="prefix for the run dir (default: gepa_natural_plan_<arm>_<schema>)")
    ...
    if args.resume is not None:
        ...
        if args.schema is None:
            p.error("--schema PATH is required for fresh runs; pass e.g. "
                    "data/natural_plan/trip/schemas/trip_fixed.jsonc")
    ...
    task = _task_from_schema(schema_path)
    run_name = args.run_name or f"gepa_natural_plan_{args.arm}_{schema_kind}"
    ...
    default_split_dir = (
        cfg.gepa_meeting_planning_main if task == "meeting"
        else cfg.gepa_trip_planning_main
    )
    default_seed_prompt = (
        cfg.meeting_seed_prompt_path if task == "meeting"
        else cfg.trip_seed_prompt_path
    )
    ...
    if prior_cfg is not None:
        ...
        split_dir = args.split_dir or default_split_dir
        ...
        seed_prompt_path = args.seed_prompt or default_seed_prompt
    ...
    print(f"[load] {len(train)} train, {len(val)} val   (task={task}, arm={args.arm})")
    ...
    print(
        f"[next] official EM: uv run python scripts/eval_natural_plan.py "
        f"--run-dir {run_dir} --task {task}"
    )
