"""GEPA_BIORED optimization: GEPA with an OA-based reward signal on BioRED.

Document-level biomedical relation extraction (text → typed coref-entities +
relations). Near-verbatim clone of ``run_gepa_docred.py`` — the adapter,
evaluator arms, OA reward, holdout machinery, and resume logic are all
dataset-agnostic; only the defaults (seed prompt, split dir, run-name prefix)
point at BioRED.
...
Four arms selected by ``--arm``:
  ...
  * ``oa_feedback_ra`` — Arm 4. Like oa_feedback but semantic referential feedback.

Single data variant (data/biored/splits/gepa_biored/main). Pair the RA schema
(data/biored/schemas/biored.jsonc) against the strict one (biored_strict.jsonc)
via --schema/--cross-schema, as in run_biored.sh. The gold entity id is the
opaque concept accession, so strict relation routing is starved while RA routes
by name+type — see research/opus48_RA_real_world_datasets_2.md.

Usage::

    uv run python scripts/run_gepa_biored.py --arm oa_score \\
        --schema data/biored/schemas/biored.jsonc \\
        --cross-schema data/biored/schemas/biored_strict.jsonc \\
...
def _run_name_suffix(schema_kind: str, dataset: str) -> str:
    ...
    The dataset's canonical stem (``"biored"`` here) maps to no suffix, so the
    ``gepa_biored_<arm>`` naming is preserved. Any other stem (``biored_strict``,
    ...) becomes ``_<stem>`` with the dataset prefix stripped if present.
    """
...
def main() -> None:
    ...
    p.add_argument(
        ...
        help=(
            ...
            "The file stem (e.g. 'biored_strict.jsonc' → 'biored_strict') is "
            "used as the run-name suffix and as the dict key under "
            "summary['holdout']['scores']. The in-repo variants are "
            "data/biored/schemas/biored.jsonc (RA on) and biored_strict.jsonc "
            "(RA off), plus their *_exact counterparts."
        ),
    )
    p.add_argument(
        "--cross-schema",
        ...
        help=(
            ...
            "summary['holdout']['scores']. If a cross-schema path matches "
            "--schema (by resolved path or stem) it is dropped with a warning."
        ),
    )
    p.add_argument(
        "--seed-prompt",
        ...
        help=(
            ...
            "data/biored/seed_prompt.txt). Inherited from config.json on "
    ...
    p.add_argument(
        ...
        help=(
            ...
            "(default: gepa_biored/main)."
        ),
    )
    p.add_argument("--run-name", type=str, default=None,
                   help="prefix for the run dir (default: gepa_biored_<arm>)")
    p.add_argument(
        "--max-metric-calls",
        type=int,
        default=None,
    ...
    # On --resume, peek the prior config FIRST so we can inherit any CLI args the
    # user didn't re-specify (--arm, --config in particular).
    ...
    if args.resume is not None:
        ...
        if args.schema is None:
            p.error(
                ...
                "data/biored/schemas/biored.jsonc"
            )
    ...
    run_name = args.run_name or (
        f"gepa_biored_{args.arm}{_run_name_suffix(schema_kind, 'biored')}"
    )
    ...
    # Resolve splits/seed/etc — from prior config on --resume, from args fresh.
    if prior_cfg is not None:
        ...
        split_dir = args.split_dir or cfg.gepa_biored_main
        ...
        seed_prompt_path = args.seed_prompt or cfg.biored_seed_prompt_path
