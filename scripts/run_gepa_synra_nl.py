"""GEPA_SYNRA_NL optimization: GEPA with an OA-based reward signal on synra_nl.

Tier-2 of the synthetic referential-alignment benchmark — see
``research/opus47_referential_alignment_dataset.md`` (R5/R6) and
``data/synra_nl/README.md`` for the M4 protocol.

Three arms selected by ``--arm``:

  * ``oa_score``    — Arm 1. Score=OA; feedback="score: X.XXX/1.0" only.
                       The recipe in ``data/synra_nl/README.md`` uses
                       this arm only (M4, score-only).
...
Usage::
    ...
    uv run python scripts/run_gepa_synra_nl.py --arm oa_score \\
        --schema data/synra_nl/schemas/synra_nl.jsonc \\
        --cross-schema data/synra_nl/schemas/synra_nl_strict.jsonc
    uv run python scripts/run_gepa_synra_nl.py --arm oa_score \\
        --schema data/synra_nl/schemas/synra_nl_strict.jsonc \\
        --cross-schema data/synra_nl/schemas/synra_nl.jsonc \\
...
def _run_name_suffix(schema_kind: str, dataset: str) -> str:
    """Run-name suffix derived from the schema's file stem.

    ``"synra_nl"`` → no suffix; ``"synra_nl_strict"`` → ``"_strict"``; any other
    stem becomes ``_<stem>`` (with dataset prefix stripped if present).
    """
...
def main() -> None:
    ...
    p.add_argument(
        ...
        help=(
            ...
            "key under summary['holdout']['scores']. In-repo variants: "
            "data/synra_nl/schemas/synra_nl.jsonc (RA on) and "
            "synra_nl_strict.jsonc (RA off)."
        ),
    )
    p.add_argument(
        "--cross-schema",
        ...
        help=(
            ...
            "Repeat to add several; per-schema aggregates land in "
            "summary['holdout']['scores']. A cross-schema path that "
            "matches --schema is dropped with a warning."
        ),
    )
    p.add_argument(
        "--seed-prompt",
        ...
        help=(
            ...
            "data/synra_nl/seed_prompt.txt). Inherited from config.json on "
    ...
    p.add_argument(
        ...
        help=(
            ...
            "(default: gepa_synra_nl/pilot)."
        ),
    )
    p.add_argument("--run-name", type=str, default=None,
                   help="prefix for the run dir (default: gepa_synra_nl_<arm>)")
    p.add_argument(
        "--max-metric-calls",
        type=int,
        default=None,
    ...
    p.add_argument(
        "--task-top-p", type=float, default=None,
        help=(
            "Optional nucleus-sampling cutoff for the task LM. Omitted by "
            "default, so the server's own default (typically the model's HF "
            "generation_config.json) applies. Has no effect at temperature=0."
        ),
    )
    p.add_argument(
        "--task-repetition-penalty", type=float, default=None,
        help=(
            "Optional vLLM repetition_penalty for the task LM (>1 discourages "
            "verbatim repeats; 1.05-1.15 is typical). Omitted by default so "
            "the server's own default applies. Sent via extra_body since it "
            "is not an OpenAI-spec parameter."
        ),
    )
    p.add_argument("--task-max-tokens", type=int, default=None,
                   help="Override task_lm max_tokens (default: YAML, else 0; 0 = no cap)")
    p.add_argument(
        "--task-retry-on-bad-json", type=int, default=0,
        help=(
            "If >0, re-call the task_lm up to N times when its response fails "
            "to parse as a JSON object (the common runaway-repetition mode). "
            "Each retry bumps temperature by --task-retry-temperature-step "
            "(capped at 2.0). 0 (default) = no retries; current behavior."
        ),
    )
    p.add_argument(
        "--task-retry-temperature-step", type=float, default=0.2,
        help=(
            "Temperature increment applied per bad-JSON retry attempt. "
            "Default: %(default)s."
        ),
    )
    ...
    if args.resume is not None:
        ...
        if args.schema is None:
            p.error(
                ...
                "data/synra_nl/schemas/synra_nl.jsonc"
            )
    ...
    run_name = args.run_name or (
        f"gepa_synra_nl_{args.arm}{_run_name_suffix(schema_kind, 'synra_nl')}"
    )
    ...
    if prior_cfg is not None:
        ...
        seed_text = prior_cfg["seed_system_prompt"]
        if (
            args.seed_prompt is not None
            and load_seed_prompt_from_path(args.seed_prompt) != seed_text
        ):
            p.error(
        ...
        split_dir = args.split_dir or cfg.gepa_synra_nl_pilot
        ...
        seed_prompt_path = args.seed_prompt or cfg.synra_nl_seed_prompt_path
        seed_text = load_seed_prompt_from_path(seed_prompt_path)
        run_dir = None
        resuming = False
    ...
    task_lm_callable = with_conversation_log(
        build_task_lm(
            ...
            top_p=args.task_top_p,
            repetition_penalty=args.task_repetition_penalty,
            max_tokens=task_max_tokens,
            json_mode=True,
            stream_to_stderr=args.stream_task_lm,
            retry_on_bad_json=args.task_retry_on_bad_json,
            retry_temperature_step=args.task_retry_temperature_step,
        ),
    ...
    try:
        result = gepa.optimize(
            seed_candidate={SYSTEM_COMPONENT: seed_text},
            trainset=train,
            valset=val,
            adapter=adapter,
