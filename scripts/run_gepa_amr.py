"""AMR (sentence → meaning graph) optimization: GEPA with an OA-based reward.

A thin sibling of ``scripts/run_gepa_sentence_ordering.py`` (same arms, adapter,
holdout, resume — all reused unchanged). AMR is the #1 real-world referential-
alignment fit: its standard metric, Smatch, *is* a best-variable-alignment graph
match, so the experiment contrasts OA's referential alignment
(``--schema .../amr_ra.jsonc``, = Smatch) against the literal-id ablation
(``.../amr_strict.jsonc``). AMR variable names are arbitrary and never given to
the model, so the ra-vs-strict gap is pure id-routing — and (unlike GraphImg/
SciERC) AMR nodes individuate well by concept + attributes, so RA is expected to
win big. Run both arms back-to-back and cross-score (see ``run_amr.sh``).

One corpus-agnostic schema pair is shared by all corpus variants; the variant is
selected purely by ``--split-dir`` / ``--seed-prompt`` (default: Little Prince).
Four arms by ``--arm``: ``oa_score`` / ``oa_feedback`` / ``oa_feedback_ra``
(semantic referential feedback) / ``oa_none``. Smatch F1
(for SOTA comparison) is computed separately by ``scripts/eval_amr.py``.
...
ARMS = {
    ...
    # oa_feedback minus structural (root/relation/node-id) feedback text; same
    # score. AMR-only ablation — see research/opus48_amr_bio_analysis.md.
    "oa_feedback_nostruct": OaFeedbackNoStructEvaluator,
...
def main() -> None:
    ...
    p.add_argument("--schema", type=Path, default=None, metavar="PATH",
                   help="OA schema (.jsonc). The stem (amr_ra/amr_strict) is the "
                        "run-name suffix and the key under holdout['scores'].")
    ...
    p.add_argument("--seed-prompt", type=Path, default=None, metavar="PATH",
                   help="Seed system prompt (default: the Little Prince seed_prompt.txt).")
    p.add_argument("--config", type=Path, default=None,
                   help="LM config YAML (default: config/experiment.yaml or "
                        "$OAEXP_CONFIG).")
    p.add_argument("--split-dir", type=Path, default=None,
                   help="Dir with train/val/test.jsonl (default: gepa_amr_littleprince/main).")
    p.add_argument("--run-name", type=str, default=None,
                   help="prefix for the run dir (default: gepa_amr_littleprince_<arm>_<schema>)")
    ...
    run_name = args.run_name or f"gepa_amr_littleprince_{args.arm}_{schema_kind}"
    ...
    if prior_cfg is not None:
        ...
        split_dir = args.split_dir or cfg.gepa_amr_littleprince_main
        ...
        seed_prompt_path = args.seed_prompt or cfg.littleprince_seed_prompt_path
    ...
    print(f"[load] {len(train)} train, {len(val)} val   (amr, arm={args.arm})")
    ...
    print(
        f"[next] official Smatch F1: uv run python "
        f"scripts/eval_amr.py --run-dir {run_dir}"
    )
